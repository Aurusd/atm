...
public class PersistUtils {
	...
	private static final Logger LOGGER = Logger.getLogger(PersistUtils.class);
...
//        serverSettings = ConverterApplication.APP_CONTEXT.getBean(ServerSettings.class);
        ...
        LOGGER.info("Creating session factory " + configurationFilename);
	...
	public void printStatistics() {
		LOGGER.info("Session close count = " + getSessionFactory().getStatistics().getSessionCloseCount());
		LOGGER.info("Session open count = " + getSessionFactory().getStatistics().getSessionOpenCount());

		LOGGER.info("Object fetch count = " + getSessionFactory().getStatistics().getEntityFetchCount());
		LOGGER.info("Object insert count = " + getSessionFactory().getStatistics().getEntityInsertCount());
		LOGGER.info("Object load count = " + getSessionFactory().getStatistics().getEntityLoadCount());
		LOGGER.info("Object delete count = " + getSessionFactory().getStatistics().getEntityDeleteCount());

		LOGGER.info("Query cache hit count = " + getSessionFactory().getStatistics().getQueryCacheHitCount());
		LOGGER.info("Query cache miss count = " + getSessionFactory().getStatistics().getQueryCacheMissCount());
		LOGGER.info("Query cache putByDayStatistic count = " + getSessionFactory().getStatistics().getQueryCachePutCount());

		LOGGER.info("Second level hit count =  " + getSessionFactory().getStatistics().getSecondLevelCacheHitCount());
		LOGGER.info("Second level miss count =  " + getSessionFactory().getStatistics().getSecondLevelCacheMissCount());
		LOGGER.info("Second level putByDayStatistic count =  " + getSessionFactory().getStatistics().getSecondLevelCachePutCount());

		for (String region : getSessionFactory().getStatistics().getSecondLevelCacheRegionNames()) {
			LOGGER.info("Second level cache region is " + region);
		}
	}
